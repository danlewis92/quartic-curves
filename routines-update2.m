...
{Computes contact points of f with lines joining extremal values (over RR), returns contact points and local image W}
    ...
    function ContactPoints(f,bit)
        ...
        for i in [1..#Points] do
            for j in [1..#Points] do
                if ptsgn[i] * ptsgn[j] eq -1 then
                    m := (Points[i,3]-Points[j,3])/(Points[i,2]-Points[j,2]);
                    l := ZZ - m*(YY - Points[j,2]) - Points[j,3]*XX;
                    param := [1,(Points[i,2]-Points[j,2])*T + Points[j,2],(Points[i,3]-Points[j,3])*T + Points[j,3]];
        ...
        W := {Vector([m[i]+m[7]: i in [1..6]]) where m:=[GF(2)|(1-Sign(Evaluate(b, p))) div 2: b in bit]: p in CPoints};
        Wsub := sub<VectorSpace(GF(2),6)|[w-w0: w in W]> where w0:=Rep(W);
        bool := (Dimension(Wsub) eq 3);
        return W,Wsub,bool;
    end function;
    
    W,Wsub,bool := ContactPoints(f,btr`bitangs[1..7]);
    Wsub := sub<VectorSpace(GF(2),6)|[w-w0: w in W]> where w0:=Rep(W);
    
    if bool eq false then
        fr,m:=MinimizeReducePlaneQuartic(f);
        fr;
        fmin:=f^m;
        FM:=fmin div GCD(Coefficients(fmin));
        BM:=[g div GCD(Coefficients(g)) where g:=b^m: b in btr`bitangs[1..7]];
        f:=FM;
        W,Wsub,bool := ContactPoints(FM,BM);
        if bool eq false then 
            error "We still have a mismatch in dimension of span of real image.";
        end if;
    end if;
    m1:=Matrix(#btr`S,1,[GF(2)|(1-Sign(s))div 2: s in btr`S]);
    m6:=DiagonalJoin([m1: j in [1..6]]);
    H1v := VectorSpace(GF(2),6);
    rho_real:=hom< btr`H1S->H1v | m6 >;
    return W, rho_real;
end intrinsic
